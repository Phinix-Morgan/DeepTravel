    ...
    <section className="section"><div className="content-width custom-trip-details">
      ...
      <aside className="custom-trip-quote">
        <Card padding="lg"><span className="eyebrow">{quoteAvailable ? "Your proposal" : "Proposal status"}</span>
          ...
          </> : <><h2>{expired ? "This proposal has expired." : request.status === "rejected" ? "This request wasn’t approved." : request.status === "cancelled" ? "This request was cancelled." : "Your proposal is in progress."}</h2><p>{expired ? "Please start a new request and we’ll be happy to create an updated proposal." : request.status === "rejected" ? "Contact Hubsafari if you would like to discuss another journey." : "Our travel designers are reviewing your details. We’ll add your pricing and notes here when it is ready."}</p>{expired && <Button as={Link} to="/custom-trip">Start a new request</Button>}</>}
        </Card>
      </aside>
    </div></section>
  </main>;
